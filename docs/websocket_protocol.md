...
- `test_case_meta`: `object` (optional) - Additional metadata about the test case. Common fields:
...
```json
{
  ...
  "test_case_id": "AuthenticationTest.LoginSuccess",
  "test_case_meta": {
    "status": "running",
...
- `test_case_id`: `string` - The test case ID this log batch belongs to
...
- `test_case_id`: `string` - The test case ID
...
```json
{
  "type": "exception",
  "run_id": "run-tree-test-174600",
  "test_case_id": "AuthenticationTest.LoginFailure",
...
### 1. Log Stream (`/ws/logs/{run_id}/{test_case_id}`)

When the test case log page connects to:

- `ws://<host>:<port>/ws/logs/{run_id}/{test_case_id}`
...
- `test_case_started` – `{ "type": "test_case_started", "run_id": "...", "test_case_id": "...", "test_case_meta": { ... }, "counts": { "passed": 0, "failed": 0, "skipped": 0, "aborted": 0 } }`
...
- `exception` – `{ "type": "exception", "run_id": "...", "test_case_id": "...", "stack_trace": { ...fields as in the exception message type above... } }`
...
```
{"event": "message_type", "run_id": "run-id", "test_case_id": "test-case-id", "ts": "2025-09-20T15:46:02.868227Z"}
```
    ...
    async with aiohttp.ClientSession() as session:
        async with session.ws_connect("ws://localhost:8080/ws/nunit") as ws:
            ...
            await ws.send_json({
                "type": "test_case_started",
                "run_id": run_id,
                "test_case_id": "MyTest.TestMethod"
            })
...
- Test case IDs can contain special characters like quotes (handled by the server)
- The server automatically handles HTML entity decoding for test case IDs
